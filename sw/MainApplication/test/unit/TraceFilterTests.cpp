...
struct TraceFilterTests : public testing::Test
{
   TraceFilterTests():
   ...
   void SetUp()
   {
      ...
      m_test_subject.reset(new TraceFilter(TEST_FILTER_ID, test_controller, TEST_BUTTON_NAME, fake_persistence));
   }
   void TearDown()
   {
   ...
   std::unique_ptr<TraceFilter> m_test_subject;
...
TEST_F(TraceFilterTests, filtering_requested_when_not_active)
{
   /**
...
TEST_F(TraceFilterTests, enabling_filter)
{
   /**
...
TEST_F(TraceFilterTests, disabling_filter)
{
   /**
    * <b>scenario</b>: Trace filter is disabled, user requested to change the color, then filter was activated and correct text was tested. <br>
...
TEST_F(TraceFilterTests, color_change_requesting_when_filter_active)
{
   /**
    * <b>scenario</b>: Trace filter is disabled, user requested to change the color, then filter was activated and correct text was tested <br>
...
TEST_F(TraceFilterTests, persistence_write_and_read)
{
   /**
   ...
   EXPECT_CALL(*GUIControllerMock_get(), setTraceFilterBackgroundColor(_, test_palette.color(QPalette::Base).rgb()));
   EXPECT_CALL(*GUIControllerMock_get(), setTraceFilterFontColor(_, test_palette.color(QPalette::Text).rgb()));
   std::unique_ptr<TraceFilter> new_test_subject = std::unique_ptr<TraceFilter>(new TraceFilter(0, test_controller, TEST_BUTTON_NAME, fake_persistence));
...
TEST_F(TraceFilterTests, refresh_ui_when_filter_disabled)
{
   /**
...
TEST_F(TraceFilterTests, refresh_ui_when_user_colors_used)
{
   /**
...
TEST_F(TraceFilterTests, refresh_ui_when_filer_active)
{
   /**
...
TEST_F(TraceFilterTests, settings_set_and_get_tests)
{
