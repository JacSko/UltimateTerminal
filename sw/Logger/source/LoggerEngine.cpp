...
void LoggerEngine::startFrontends()
{
   ...
   if (SETTING_GET_BOOL(Logger_supportStdoutLogging))
   {
   ...
   {
      m_frontends.emplace_back(new LoggerSocketWriter(SETTING_GET_U32(Logger_socketPort)));
   }
...
void LoggerEngine::refreshGroupLevelSettings()
{
   for (uint32_t i = 0; i < LOGGER_GROUP_MAX; i++)
   {
      ...
      {
         /* if setting found */
         m_group_levels[i] = getLevelFromString(SETTING_GET_STRING(id));
      }
   }
}
...
{
   if (logger_writer != ALL_WRITERS)
   {
      for (auto& frontend : m_frontends)
      {
         if(frontend->getType() == logger_writer)
         {
